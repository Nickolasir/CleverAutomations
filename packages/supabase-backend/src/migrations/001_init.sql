-- =============================================================================
-- Clever Automations - Migration 001: Initial Schema
-- =============================================================================
...
-- Convert to hypertable (safe if already a hypertable)
...
CREATE POLICY users_insert ON users
  FOR INSERT WITH CHECK (tenant_id = auth.tenant_id() AND role_at_least('admin'));
CREATE POLICY users_update ON users
  FOR UPDATE USING (tenant_id = auth.tenant_id() AND (role_at_least('admin') OR id = auth.user_id()));
CREATE POLICY users_delete ON users
  FOR DELETE USING (tenant_id = auth.tenant_id() AND role_at_least('owner'));

-- ---- DEVICES ----
CREATE POLICY devices_select_members ON devices
  FOR SELECT USING (tenant_id = auth.tenant_id() AND role_at_least('resident'));
...
CREATE POLICY devices_insert ON devices
  FOR INSERT WITH CHECK (tenant_id = auth.tenant_id() AND role_at_least('manager'));
CREATE POLICY devices_update ON devices
  FOR UPDATE USING (tenant_id = auth.tenant_id() AND role_at_least('manager'));
CREATE POLICY devices_delete ON devices
  FOR DELETE USING (tenant_id = auth.tenant_id() AND role_at_least('admin'));
...
CREATE POLICY voice_sessions_select_guests ON voice_sessions
  FOR SELECT USING (tenant_id = auth.tenant_id() AND auth.user_role() = 'guest' AND user_id = auth.user_id());
...
CREATE POLICY reservations_select ON reservations
  FOR SELECT USING (tenant_id = auth.tenant_id() AND role_at_least('manager'));
CREATE POLICY reservations_select_guests ON reservations
  FOR SELECT USING (
    ...
    AND guest_profile_id IN (
      SELECT gp.id FROM guest_profiles gp WHERE gp.tenant_id = auth.tenant_id()
    )
  );
...
CREATE POLICY guest_profiles_select_guests ON guest_profiles
  FOR SELECT USING (
    ...
    AND id IN (
      ...
      WHERE r.tenant_id = auth.tenant_id() AND r.status = 'active'
    )
  );
...
CREATE POLICY guest_profiles_delete ON guest_profiles
  FOR DELETE USING (tenant_id = auth.tenant_id() AND role_at_least('manager'));
